...
# ====== Open log file ======
log_file = open("detail.out", "a")  # modo append para continuar registrando
...
if not cif_files:
    ...
    log_file.write("No .cif files found in the specified structures directory.\n")
    log_file.close()
    exit()
...
for cif_file in cif_files:
    ...
    msg = f"Processing structure: {cif_file}"
    print(f"\n{msg}")
    ...
    try:
        ...
        df_partial = pd.DataFrame(all_descriptors)
        df_partial.to_csv(output_descritor_csv, index=False)

        msg = f"Done: {cif_file} in {time.time() - start_time:.2f} seconds"
        print(msg)
        log_file.write(f"{msg}\n")

        msg2 = f"Partial descriptors saved to: {output_descritor_csv}"
        print(msg2)
        log_file.write(f"{msg2}\n")
        log_file.flush()

    except Exception as e:
        err_msg = f"Error processing {cif_file}: {str(e)}"
        print(err_msg)
